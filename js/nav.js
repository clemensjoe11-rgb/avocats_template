(() => {
  ...
  if(navEl){
    ...
    navEl.addEventListener('click', e=>{
      if(e.target.matches('.nav-link')){ const c=bootstrap.Collapse.getInstance(navEl); c&&c.hide(); }
    });
  }
  const setOffset=()=>document.documentElement.style.setProperty('--nav-h', nav.offsetHeight+'px');
  addEventListener('load', setOffset); addEventListener('resize', setOffset); setOffset();
})();